...
U8 get_alive_neighbors(char *cell, U64 x, U64 y, U64 width, U64 height) {
    ...
    if (y > 0 && cell[-width] == '#')
        count += 1; // up
    if (y + 1 < height && cell[width] == '#')
        count += 1; // down
    if (x > 0 && y > 0 && cell[-1 - width] == '#')
        count += 1; // left+up
    if (x + 1 < width && y > 0 && cell[1 - width] == '#')
        count += 1; // right+up
    if (x > 0 && y + 1 < height && cell[-1 + width] == '#')
        count += 1; // left+down
    if (x + 1 < width && y + 1 < height && cell[1 + width] == '#')
        count += 1; // right+down
